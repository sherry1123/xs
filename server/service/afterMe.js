    ...
    async getNodeList(param) {
        ...
        if (!res.errorId) {
            for (let i of Object.keys(res.data)) {
                ...
                res.data[i] = { hostname, ip, service: service.filter(i => (i !== 'client')).map(i => (i === 'meta' ? 'metadata' : i)), isPureMgmt: service.length === 1 && service.includes('mgmt'), status, cpuUsage: `${cpuUsage.toFixed(2)}%`, memoryUsage: `${memUsage.toFixed(2)}%`, space: { total: spaceTotal, used: spaceUsed, free: spaceFree, usage: `${(spaceUsed / spaceTotal * 100).toFixed(2)}%` } };
            }
