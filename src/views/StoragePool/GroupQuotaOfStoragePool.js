	...
	delete (groupQuota){
		...
		const modal = Modal.confirm({
			...
			content: <div style={{fontSize: 12}}>
				...
				<p>{lang(`建议：在执行该操作前，请确保您选择了正确的本地认证用户组和存储池，并且该用户在该存储池下确实已不需要做配额限制。`, `A suggestion: before executing this operation, ensure that you select the right storage pool and it's no longer necessary.`)}</p>
			</div>,
			keyboard: false,
			...
			onOk: async () => {
				...
				try {
					await httpRequests.deleteGroupQuotaFromStoragePool({poolId, ...groupQuota});
					httpRequests.getGroupQuotasOfStoragePoolById(this.state.poolId);
